...
public class MembreDAO extends BaseHibernateDAO {
	...
	public Membre findById(java.lang.Integer id) {
		...
		try {
			Membre instance = (Membre) getSession().get("hiber.Membre", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Membre> findByExample(Membre instance) {
		...
		try {
			List<Membre> results = (List<Membre>) getSession()
					.createCriteria("hiber.Membre").add(create(instance))
					.list();
	...
	public List<Membre> findByConjoint(Object conjoint) {
		return findByProperty(CONJOINT, conjoint);
	}
